...
import ImageSearchItem from "../Model/Image/ImageSearchItem";
 ...
 * 搜索镜像
 */
function searchImage(term: string): Promise<Base<Array<ImageSearchItem>>> {
    return httpRequest.post(`/images/search?term=${term}`).then(data => data.data);
}
