...
export default {
  ...
  computed: {
    ...
    async exposeContainerToLocal(containerId) {
      let key = guid()
      this.$message.loading({content: "正在备份容器数据，请稍后....", key, duration: 0});
      let res = await containerApi.exposeContainerToLocal(containerId)
      let {Code, Msg, Data} = res.data;
      if (Code === 'OK') {
        this.$message.info({content: `容器已成功备份到: ${Data}`, key, duration: 5});
      } else {
    ...
    openTerminal(state, containerId) {
      ...
      let openUrl = function () {
        let routeUrl = router.resolve({
          ...
          query: {containerId: containerId, title: 'Docker终端控制台'}
        });
    ...
    openFileManagementModal(state, containerId) {
      ...
      let openUrl = function () {
        let routeUrl = router.resolve({
          path: "/terminal/file",
          query: {containerId: containerId, title: 'Docker文件管理'}
        });
        window.open(routeUrl.href, '_blank');
      }
    ...
    openContainerMonitor(state, containerId) {
      ...
      let routeUrl = this.$router.resolve({
        ...
        query: {containerId: containerId, title: 'Docker容器状态'}
      });
      window.open(routeUrl.href, '_blank');
    }
  }
